...
class xmlToArrayParser {
    ...
    private function tag_close($parser, $tag) {
        ...
        if (isset($current['cdata']) && count($current) == 1) {$current = $current['cdata'];} else if (empty($current['cdata'])) {unset($current['cdata']);}
    }

    /** Converts a single element item into [element[0]] if a second element of the same name is encountered. */
